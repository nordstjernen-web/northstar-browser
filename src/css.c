...
static const ns_css_property_meta kProperty[NS_CSS_PROP_COUNT] = {
    ...
    [NS_CSS_ANIMATION_NAME]       = P("animation-name"),
    [NS_CSS_ANIMATION_TIMING_FUNCTION] = P("animation-timing-function"),
    [NS_CSS_ANIMATION_ITERATION_COUNT] = P("animation-iteration-count"),
    [NS_CSS_ANIMATION_DIRECTION]  = P("animation-direction"),
...
ns_inline_style_set(const char *style, const char *prop, const char *raw_value)
{
    ...
    if (!style || !*style) {
        ...
        const int *qids = inline_quad_ids(prop);
        if (qids) {
            char *qexp = inline_quad_expanded(prop, value);
            g_free(value);
            return qexp ? qexp : g_strdup("");
        }
        if (g_ascii_strcasecmp(prop, "all") != 0 && inline_logical_group(ns_css_prop_id(prop)) == 0) {
            char *res = g_strdup_printf("%s: %s", prop, value);
            g_free(value);
            return res;
        }
    }
